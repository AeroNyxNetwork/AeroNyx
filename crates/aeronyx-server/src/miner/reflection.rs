...
//!
//! ## v2.5.3+ArtifactChain: Step 10 Artifact Version Chain (DEV-TASK-001)
//! Step 10 code block extraction now:
//!   1. Infers filename from surrounding context (infer_filename helper)
//!   2. Queries get_latest_artifact_version() to build version chain
//!   3. Deduplicates by content_hash (skip if content unchanged)
...
//! v2.4.0-GraphCognition Phase B - 🌟 Full Step 7-11 implementations
//! v2.4.0-GraphCognition Phase B Fixes - 🌟 Step 8 project_id back-fill; Step 7
//!   stopword filter; BUG-FIX-1/2/3
//! v2.4.0+Search            - 🌟 Step 10 session title generation
...
impl ReflectionMiner {
    ...
    // ============================================
    // v2.4.0: Cognitive Graph Steps (Phase B)
    // ============================================
                        ...
                        for (episode_id, _role) in &ep_links {
                            if episode_id.starts_with("ep_") {
                                ...
                                if let Some(last_underscore) = rest.rfind('_') {
                                    project_session_ids.insert(rest[..last_underscore].to_string());
                                }
                            }
                        }
                    }
    ...
    /// Code block extraction now:
    /// 1. Infers filename from surrounding text (comment on preceding line or
    ///    first line of code block). Falls back to `snippet_{hash[:8]}.{ext}`.
    /// 2. Queries `get_latest_artifact_version()` to build version chain.
    ...
    async fn step_10_session_summary(&self) {
        ...
        for session in &pending {
            ...
            for cap in code_regex.captures_iter(&full_text) {
                ...
                // v2.5.3: Infer filename from surrounding context
                let code_start_pos = cap.get(0).map(|m| m.start()).unwrap_or(0);
                ...
                if let Err(e) = self.storage.insert_artifact(
                    ...
                    debug!(
                        ...
                        parent = ?latest_artifact_id,
                        "[MINER_S10] Artifact v{} saved: {}", new_version, filename
                    );
                    total_artifacts += 1;
                }
            }
...
// ============================================
// v2.5.3+ArtifactChain: Filename Inference Helpers
// ============================================

/// Infer filename for a code block from surrounding text context.
///
/// ## Strategy (by priority)
/// 1. Preceding line: comment containing a filename (e.g. `// auth.rs`, `# main.py`)
/// 2. First line inside the code block: same pattern
...
fn infer_filename(
    ...
    if code_start_pos > 0 {
        let before = &full_text[..code_start_pos];
        if let Some(last_line) = before.lines().rev().find(|l| !l.trim().is_empty()) {
            if let Some(name) = extract_filename_from_line(last_line) {
                return name;
            }
        }
    }

    // Strategy 2: first line inside the code block (after the opening fence line)
    let after_fence = &full_text[code_start_pos..];
    if let Some(second_line) = after_fence.lines().nth(1) {
        if let Some(name) = extract_filename_from_line(second_line) {
            return name;
        }
    }

    // Strategy 3: fallback with hash prefix for uniqueness
    format!("snippet_{}.{}", &content_hash[..8], ext)
}

/// Extract a filename from a single line of text.
///
/// Matches patterns:
/// - `// auth.rs`
/// - `# main.py`
/// - `File: storage.rs`
/// - `// File: handlers/mpi.rs`
/// - `<!-- config.yaml -->`
///
/// Returns `None` if no recognizable filename pattern is found.
///
/// v2.5.3+ArtifactChain
fn extract_filename_from_line(line: &str) -> Option<String> {
    ...
    for cap in re.captures_iter(line) {
        ...
        if KNOWN_EXTS.contains(&file_ext) {
