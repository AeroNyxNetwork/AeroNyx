...
impl MemoryStorage {
    ...
    pub async fn get_entities_by_owner(
        ...
        if let Some(et) = entity_type {
            ...
            stmt.query_map(params![owner.as_slice(), et, limit as i64], |row| {
                Ok(EntityRow {
                    entity_id: row.get(0)?,
                    name: row.get(1)?,
                    name_normalized: row.get(2)?,
    ...
    pub async fn get_entities_cached(&self, owner: &[u8; 32]) -> HashMap<String, String> {
        ...
        let mut stmt = match conn
            .prepare("SELECT name_normalized, entity_id FROM entities WHERE owner = ?1")
        {
            Ok(s) => s,
...
impl MemoryStorage {
    ...
    pub async fn get_edges_for_entity(
        ...
        stmt.query_map(params![owner.as_slice(), entity_id], |row| {
            Ok(KnowledgeEdgeRow {
                edge_id: row.get(0)?,
                source_id: row.get(1)?,
                target_id: row.get(2)?,
...
impl MemoryStorage {
    ...
    pub async fn get_entities_for_episode(&self, episode_id: &str) -> Vec<(String, String)> {
        let conn = self.conn.lock().await;
        let mut stmt =
            match conn.prepare("SELECT entity_id, role FROM episode_edges WHERE episode_id = ?1") {
                Ok(s) => s,
                Err(_) => return Vec::new(),
            };
    ...
    pub async fn get_episodes_for_entity(&self, entity_id: &str) -> Vec<(String, String)> {
        let conn = self.conn.lock().await;
        let mut stmt =
            match conn.prepare("SELECT episode_id, role FROM episode_edges WHERE entity_id = ?1") {
                Ok(s) => s,
                Err(_) => return Vec::new(),
            };
...
impl MemoryStorage {
    ...
    pub async fn get_entities_in_community(
        &self,
        community_id: &str,
        owner: &[u8; 32],
    ) -> Vec<EntityRow> {
        ...
        stmt.query_map(params![owner.as_slice(), community_id], |row| {
            Ok(EntityRow {
                entity_id: row.get(0)?,
                name: row.get(1)?,
                name_normalized: row.get(2)?,
    ...
    pub async fn get_communities_with_new_entities(
        &self,
        owner: &[u8; 32],
        since: i64,
    ) -> Vec<String> {
        ...
        stmt.query_map(params![owner.as_slice(), since], |row| {
            row.get::<_, String>(0)
        })
        .map(|rows| rows.filter_map(|r| r.ok()).collect())
        .unwrap_or_default()
...
impl MemoryStorage {
    ...
    pub async fn get_projects(
        ...
        if let Some(s) = status {
            ...
            stmt.query_map(params![owner.as_slice(), s, limit as i64], |row| {
                Ok(ProjectRow {
...
impl MemoryStorage {
    ...
    pub async fn get_sessions_for_project(
        ...
        stmt.query_map(
            params![project_id, limit as i64, offset as i64, owner.as_slice()],
            |row| {
                Ok(SessionRow {
                    session_id: row.get(0)?,
                    project_id: row.get(1)?,
...
impl MemoryStorage {
    ...
    pub async fn get_artifacts_for_session(
        &self,
        session_id: &str,
        owner: &[u8; 32],
    ) -> Vec<ArtifactRow> {
        ...
        stmt.query_map(params![session_id, owner.as_slice()], |row| {
            Ok(ArtifactRow {
    ...
    pub async fn get_artifact_versions(
        ...
        stmt.query_map(params![owner.as_slice(), filename], |row| {
            Ok(ArtifactRow {
                artifact_id: row.get(0)?,
                session_id: row.get(1)?,
                project_id: row.get(2)?,
...
mod tests {
    ...
    async fn test_entity_upsert_existing_increments() {
        let s = MemoryStorage::open(":memory:", None).unwrap();
        let owner = [0xAA; 32];
        s.upsert_entity("ent_jwt", &owner, "JWT", "jwt", "technology", None, None)
            .await
            .unwrap();
        let is_new = s
            .upsert_entity(
    ...
    async fn test_knowledge_edge_invalidation() {
        ...
        let eid = s
            .insert_knowledge_edge(
                &owner, "ent_auth", "ent_jwt", "USES", None, 1.0, 0.9, None, now, None,
            )
            .await
            .unwrap();
    ...
    async fn test_episode_edge_bidirectional() {
        let s = MemoryStorage::open(":memory:", None).unwrap();
        let owner = [0xAA; 32];
        s.insert_episode_edge(&owner, "ep_001", "ent_jwt", "mentioned")
            .await
            .unwrap();
        s.insert_episode_edge(&owner, "ep_001", "ent_auth", "produced")
            .await
            .unwrap();
    ...
    async fn test_community_upsert_and_query() {
        ...
        s.upsert_community(
            ...
            &owner,
            "Auth System",
            Some("Authentication components"),
            None,
            5,
        )
        .await
    ...
    async fn test_session_upsert_and_title() {
        ...
        let now = SystemTime::now()
            ...
            .unwrap()
            .as_secs() as i64;
        s.upsert_session("sess_001", &owner, None, "code", now, 15)
            .await
            .unwrap();
        ...
        s.update_session_summary("sess_001", "Topics: JWT", None, Some("Project Alpha: JWT"))
            .await;
        ...
        s.update_session_summary("sess_001", "Topics: JWT, RS256", None, None)
            .await;
    ...
    async fn test_project_upsert_and_sessions() {
        ...
        let now = SystemTime::now()
            ...
            .unwrap()
            .as_secs() as i64;
        s.upsert_community("comm_1", &owner, "Project B", None, None, 3)
            .await
            .unwrap();
        s.upsert_project(
            "comm_1",
        ...
        .await
        .unwrap();
        s.upsert_session("sess_001", &owner, Some("comm_1"), "code", now, 10)
            .await
            .unwrap();
    ...
    async fn test_sessions_for_project_pagination() {
        ...
        let now = SystemTime::now()
            ...
            .unwrap()
            .as_secs() as i64;
        s.upsert_community("comm_1", &owner, "Proj", None, None, 1)
            .await
            .unwrap();
        for i in 0..5u64 {
            s.upsert_session(
        ...
        let isolated = s
            .get_sessions_for_project("comm_1", 10, 0, &other_owner)
            .await;
        assert!(
            isolated.is_empty(),
            "Different owner must not see other's sessions"
        );
    }
    ...
    async fn test_graph_stats() {
        ...
        let now = SystemTime::now()
            ...
            .unwrap()
            .as_secs() as i64;
        s.upsert_entity("ent_1", &owner, "JWT", "jwt", "technology", None, None)
            .await
            .unwrap();
        s.upsert_entity("ent_2", &owner, "auth", "auth", "module", None, None)
            .await
            .unwrap();
        s.insert_knowledge_edge(
            &owner, "ent_2", "ent_1", "USES", None, 1.0, 0.9, None, now, None,
        )
        .await
        .unwrap();
        s.upsert_community("comm_1", &owner, "Auth", None, None, 2)
            .await
            .unwrap();
