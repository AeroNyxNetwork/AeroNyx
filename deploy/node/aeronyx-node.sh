...
Commands:
  ...
  join       Install and submit a public node for Stage-A admission without registration.
...
Common options:
  ...
  join:
    ...
    --check-only           Validate local readiness without POST or host changes.
    --json|--json-only     Emit aggregate Stage-A result; never peer identities.
...
Examples:
  ...
  sudo ./deploy/node/aeronyx-node.sh join --seed http://PUBLIC_SEED_IP:8422 --public-endpoint http://YOUR_PUBLIC_IP:8422
...
parse_args() {
    ...
    while [ "$#" -gt 0 ]; do
        case "$1" in
            ...
            --seed) JOIN_SEEDS+=("${2:?missing value}"); shift 2 ;;
            --public-endpoint) JOIN_PUBLIC_ENDPOINT="${2:?missing value}"; shift 2 ;;
            --join-timeout) JOIN_TIMEOUT="${2:?missing value}"; shift 2 ;;
            --check-only) JOIN_CHECK_ONLY=1; shift ;;
            --yes|-y) YES=1; shift ;;
...
# [PERMISSIONLESS-NODE-JOIN-MODULE 2026-09-24 by Codex] Keep the unified
# command wrapper thin; sourcing defines functions but performs no join work.
[ -r "${SCRIPT_DIR}/lib/operator_join.sh" ] || die "operator join module is unavailable"
source "${SCRIPT_DIR}/lib/operator_join.sh"
...
# [PERMISSIONLESS-NODE-JOIN 2026-09-24 by Codex] Source-safe focused shell
# fixtures can exercise the exact operator functions without host side effects.
